...
- **Grid bulk archive actions** — the fork bulk-selection path is back on the normal index grid. Normal-click an archive card to select/deselect it; right-long-click an already selected archive card to open bulk actions such as `Run Batch Operations`, category add, delete, remove, and clear. Upstream MSM/carousel/Tankoubon selection code is still retained for merge compatibility, but the fork user path hides those controls and does not expose Tankoubon merge from grid selection. The upstream-friendly structure is documented in [`docs/local-features/library-ux-plugins-themes.md`](docs/local-features/library-ux-plugins-themes.md).
