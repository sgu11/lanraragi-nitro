...
- **Reader predecode window** — the next one or two reader pages retain decoded `Image` objects so Windows Chromium does not repeat 6 MP WebP decode work on each page turn. The decoded window is capped at two images and protected inside the existing eight-entry preload LRU.
