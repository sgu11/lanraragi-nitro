...
- **Reader module-graph preload** — the Reader template advertises `reader.js`, the large `reader_common.js` runtime, and `i18n.js` with `modulepreload`, allowing Chromium to fetch and compile the import graph before the inline module bootstrap reaches it.
