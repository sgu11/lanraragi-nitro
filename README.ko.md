...
- **Blank border crop policy**: libvips를 우선 사용하고 cover/landscape spread page는 crop하지 않음. Komikku를 참고한 v5 per-edge light/dark strip detection과 outer 2px edge-noise guard를 사용함. Crop 후보는 byte 크기와 무관하게 cropped area percentage로만 판단함. Page 면적을 5% 미만으로 줄이면 no-crop 처리하고, 의미 있는 border 제거는 re-encode byte가 커져도 crop variant로 cache함. Crop timing은 metrics에 기록함.
