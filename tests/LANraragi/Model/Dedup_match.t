...
note("match: bucket by page count, score within tolerance, store pairs <= cap");
{
    ...
    $redis->mock('smembers', sub { $membership_calls{smembers}++; keys %dismissed });
    $redis->mock('zrange',   sub { $membership_calls{zrange}++; () });
    $redis->mock('sismember',sub { die 'per-candidate SISMEMBER must not be used' });
    $redis->mock('zscore',   sub { die 'per-candidate ZSCORE must not be used' });
    ...
    is($membership_calls{smembers}, 1, "dismissed membership is preloaded once");
    is($membership_calls{zrange},   1, "existing deck membership is preloaded once");
...
note("match: dismissed pairs are skipped");
{
    ...
    $redis->mock('del',      sub { 1 });
    $redis->mock('smembers', sub { keys %dismissed });
    $redis->mock('zrange',   sub { () });
...
note("match: zscore guard skips pairs already in the deck");
{
    ...
    $redis->mock('del',      sub { 1 });
    $redis->mock('smembers', sub { () });
    $redis->mock('zrange',   sub { keys %already_in_deck });
    $redis->mock('sismember',sub { 0 });
