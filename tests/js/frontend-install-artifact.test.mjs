...
test("clean frontend install builds the Swiper artifact consumed through the import map", async (t) => {
    ...
    const entry = join(new URL("node_modules", root).pathname, entryMatch[1].replace(/^\//, ""));
    ...
    const build = spawnSync("npm", ["exec", "esbuild", "--", "--bundle", "--format=esm", "--minify",
        "--external:react", `--outdir=${outdir}`, entry], { cwd: new URL(root), encoding: "utf8" });
